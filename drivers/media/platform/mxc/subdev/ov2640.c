...
enum ov2640_mode {
	ov2640_mode_min = 0,
	ov2640_mode_1600_1120 = 0,
	ov2640_mode_800_600 = 1,
	ov2640_mode_max = 1
};
...
struct ov2640_mode_info {
	enum ov2640_mode mode;
	u32 width;
	u32 height;
...
static int ov2640_framerates[] = {
	[0] = 10,
	[1] = 30,
};

static const struct ov2640_datafmt ov2640_colour_fmts[] = {
...
static struct ov2640_mode_info ov2640_mode_info_data[ov2640_mode_max + 1] = {
	{
		ov2640_mode_1600_1120,      1600,  1120,
		ov2640_setting_1600_1120,
		ARRAY_SIZE(ov2640_setting_1600_1120)	/* fps 10 */
	},
	{
		ov2640_mode_800_600,     800,  600,
		ov2640_setting_800_600,
		ARRAY_SIZE(ov2640_setting_800_600)		/* fps 30 */
	},
};
...
struct sensor {
	...
	const struct ov2640_datafmt	*fmt;
	bool on;
 ...
 * General functions
 */
static struct sensor *to_ov2640(const struct i2c_client *client)
{
	return container_of(i2c_get_clientdata(client), struct sensor,
			    subdev);
}

#ifdef CONFIG_VIDEO_ADV_DEBUG
static int ov2640_get_register(struct v4l2_subdev *sd,
...
static int ov2640_set_register(struct v4l2_subdev *sd,
...
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct sensor *sensor = to_ov2640(client);
 ...
 */
static int ov2640_g_parm(struct v4l2_subdev *sd, struct v4l2_streamparm *a)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct sensor *sensor = to_ov2640(client);
 ...
 */
static int ov2640_s_parm(struct v4l2_subdev *sd, struct v4l2_streamparm *a)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct sensor *sensor = to_ov2640(client);
...
static const struct ov2640_datafmt
			*ov2640_find_datafmt(u32 code)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(ov2640_colour_fmts); i++)
		if (ov2640_colour_fmts[i].code == code)
			return ov2640_colour_fmts + i;
...
{
	const struct ov2640_datafmt *fmt = ov2640_find_datafmt(mf->code);

	if (!fmt) {
		mf->code	= ov2640_colour_fmts[0].code;
		mf->colorspace	= ov2640_colour_fmts[0].colorspace;
	}
			...
			struct v4l2_mbus_framefmt *mf)
{
	...
	if (!ov2640_find_datafmt(mf->code))
		return -EINVAL;

	ov2640_try_fmt(sd, mf);
	sensor->fmt = ov2640_find_datafmt(mf->code);
			...
			struct v4l2_mbus_framefmt *mf)
{
	...
	const struct ov2640_datafmt *fmt = sensor->fmt;
...
static int ov2640_enum_fmt(struct v4l2_subdev *sd, unsigned int index,
			   u32 *code)
{
	if (index >= ARRAY_SIZE(ov2640_colour_fmts))
		return -EINVAL;

	*code = ov2640_colour_fmts[index].code;
	return 0;
}
 ...
 */
static int ov2640_s_fmt_cap(struct v4l2_int_device *s, struct v4l2_format *f)
{
...
/*!
 * ov2640_enum_framesizes - V4L2 sensor interface handler for
 ...
 */
static int ov2640_enum_framesizes(struct v4l2_subdev *sd,
			       struct v4l2_subdev_pad_config *cfg,
			       struct v4l2_subdev_frame_size_enum *fse)
{
	if (fse->index > ov2640_mode_max)
		return -EINVAL;

	fse->max_width = ov2640_mode_info_data[fse->index].width;
	fse->min_width = fse->max_width;
	fse->max_height = ov2640_mode_info_data[fse->index].height;
	fse->min_height = fse->max_height;
	return 0;
}

/*!
 * ov2640_enum_frameintervals - V4L2 sensor interface handler for
 ...
 * Return 0 if successful, otherwise -EINVAL.
 */
static int ov2640_enum_frameintervals(struct v4l2_subdev *sd,
		struct v4l2_subdev_pad_config *cfg,
		struct v4l2_subdev_frame_interval_enum *fie)
{
	if (fie->index < 0 || fie->index > ov2640_mode_max)
		return -EINVAL;
	...
	fie->interval.denominator = ov2640_framerates[fie->index];

	return 0;
}


static int ov2640_set_clk_rate(void)
{
	u32 tgt_xclk;	/* target xclk */
	int ret;
	...
	pr_debug("   Setting mclk to %d MHz\n", tgt_xclk / 1000000);
	ret = clk_set_rate(ov2640_data.sensor_clk, ov2640_data.mclk);
	if (ret < 0)
		pr_debug("set rate filed, rate=%d\n", ov2640_data.mclk);
	return ret;
}

static inline void ov2640_power_down(int enable)
{
...
static inline void ov2640_reset(void)
{
	/* camera reset */
...
static int init_device(void)
{
	...
	u32 tgt_xclk;	/* target xclk */
	u32 tgt_fps;	/* target frames per secound */
	enum ov2640_frame_rate frame_rate;
#endif
	int ret;
	...
	tgt_fps = ov2640_data.streamcap.timeperframe.denominator /
		  ov2640_data.streamcap.timeperframe.numerator;

	if (tgt_fps == 15)
		frame_rate = ov2640_15_fps;
	else if (tgt_fps == 30)
		frame_rate = ov2640_30_fps;
	else
		return -EINVAL; /* Only support 15fps or 30fps now. */
#endif

	ret = ov2640_init_mode(&ov2640_data);
...
static const struct v4l2_subdev_pad_ops ov2640_subdev_pad_ops = {
	.enum_frame_size       = ov2640_enum_framesizes,
	.enum_frame_interval   = ov2640_enum_frameintervals,
};

static struct v4l2_subdev_core_ops ov2640_subdev_core_ops = {
	.s_power	= ov2640_s_power,
#ifdef CONFIG_VIDEO_ADV_DEBUG
	.g_register	= ov2640_get_register,
	.s_register	= ov2640_set_register,
#endif
};
			...
			const struct i2c_device_id *id)
{
	...
	ov2640_data.sensor_clk = devm_clk_get(dev, "csi_mclk");
	if (IS_ERR(ov2640_data.sensor_clk)) {
		dev_err(dev, "get mclk failed\n");
		return PTR_ERR(ov2640_data.sensor_clk);
	}
	...
	clk_prepare_enable(ov2640_data.sensor_clk);
	...
	retval = init_device();
	if (retval < 0) {
		clk_disable_unprepare(ov2640_data.sensor_clk);
		pr_warning("camera ov2640 init failed\n");
		ov2640_power_down(1);
		return retval;
	}

	clk_disable(ov2640_data.sensor_clk);

	v4l2_i2c_subdev_init(&ov2640_data.subdev, client, &ov2640_subdev_ops);

	retval = v4l2_async_register_subdev(&ov2640_data.subdev);
	if (retval < 0)
		dev_err(&client->dev,
...
static int ov2640_remove(struct i2c_client *client)
{
	...
	clk_unprepare(ov2640_data.sensor_clk);
	ov2640_power_down(1);
